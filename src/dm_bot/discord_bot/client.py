...
class DiscordDmBot(commands.Bot):
    ...
    def _register_commands(self) -> None:
        @self.tree.command(
            name="setup", description="Check runtime setup and model availability"
        )
        ...
        @self.tree.command(
            name="bind_archive_channel",
            description="Bind this channel as the archive/profile channel",
        )
        async def bind_archive_channel(interaction: discord.Interaction) -> None:
            await self.handlers.bind_archive_channel(interaction)

        @self.tree.command(
            name="bind_trace_channel",
            description="Bind this channel as the keeper trace channel",
        )
        ...
        @self.tree.command(
            ...
            description="Bind this channel as the admin management channel",
        )
        ...
        @self.tree.command(
            name="leave_campaign",
            description="Leave the campaign bound to this channel or thread",
        )
        async def leave_campaign(interaction: discord.Interaction) -> None:
            await self.handlers.leave_campaign(interaction)

        @self.tree.command(
            name="set_role", description="Set your scenario role or template"
        )
        ...
        @self.tree.command(
            name="start_builder",
            description="Start conversational COC character creation",
        )
        @app_commands.describe(visibility="private or public")
        async def start_builder(
            interaction: discord.Interaction, visibility: str = "private"
        ) -> None:
            await self.handlers.start_character_builder(
                interaction, visibility=visibility
            )

        @self.tree.command(
            name="builder_reply", description="Answer the current builder question"
        )
        ...
        @self.tree.command(
            name="profiles", description="List your archive investigator profiles"
        )
        ...
        @self.tree.command(
            name="profile_detail",
            description="Show one archive investigator profile in detail",
        )
        ...
        @self.tree.command(
            ...
            description="Archive one of your long-lived profiles",
        )
        @app_commands.describe(profile_id="Archive profile id")
        ...
        @self.tree.command(
            ...
            description="Activate one of your archived profiles",
        )
        @app_commands.describe(profile_id="Archive profile id")
        ...
        @self.tree.command(
            ...
            description="Import a snapshot character for the current player",
        )
        @app_commands.describe(
            provider="Character source provider",
            external_id="External character identifier",
        )
        async def import_character(
            interaction: discord.Interaction, provider: str, external_id: str
        ) -> None:
            await self.handlers.import_character(
                interaction, provider=provider, external_id=external_id
            )

        @self.tree.command(
            name="enter_scene", description="Enter multi-character scene mode"
        )
        ...
        @self.tree.command(
            name="end_scene", description="Return from scene mode to DM mode"
        )
        ...
        async def ready(
            interaction: discord.Interaction, character_name: str = ""
        ) -> None:
            await self.handlers.ready_for_adventure(
                interaction, character_name=character_name
            )

        @self.tree.command(
            name="start-session",
            description="Start the session as admin (requires all players ready)",
        )
        async def start_session(interaction: discord.Interaction) -> None:
            await self.handlers.start_session(interaction)

        @self.tree.command(
            name="complete-onboarding",
            description="Confirm you have read the onboarding rules",
        )
        ...
        @self.tree.command(
            name="check", description="Resolve an ability or skill check"
        )
        @app_commands.describe(
            label="Check label",
        ...
        async def check(
            ...
            await self.handlers.check_roll(
                interaction, label=label, modifier=modifier, advantage=advantage
            )
        ...
        @self.tree.command(
            name="attack", description="Resolve an attack roll with optional damage"
        )
        @app_commands.describe(
        ...
        @self.tree.command(
            name="debug_status", description="Show recent trace-linked runtime events"
        )
        @app_commands.describe(campaign_id="Campaign identifier")
        ...
        @self.tree.command(
            name="coc_assets", description="Show discovered local COC assets"
        )
