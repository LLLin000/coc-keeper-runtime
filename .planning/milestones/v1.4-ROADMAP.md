...
Milestone `v1.0` established the Discord-first local-DM runtime with deterministic rules, persistence, diagnostics, and a starter packaged adventure. Milestone `v1.1` introduced a formal module runtime and shipped `疯狂之馆` as the first structured full-length module. Milestone `v1.2` added ready-gated startup, mature dice integration, and true Discord streaming. Milestone `v1.3` polished live-play feel through structured judgement, bounded guidance, and keeper-style scene framing. Milestone `v1.4` changed the module representation itself by introducing room graphs, AI-first extraction drafts, and location-driven play for `疯狂之馆`.
...
## Milestone v1.4 Delivery Summary

### Phase 16: Room Graph Runtime Foundations
**Delivered**:
- Formal adventure packages now support `locations`, `start_location_id`, and explicit room-to-room connections.
- Runtime snapshot now exposes `current_location` and `reachable_locations`.
- Existing narration and persistence continued to work on top of the new state shape instead of requiring a separate stack.

### Phase 17: AI Extraction For Room Graphs And Trigger Trees
**Delivered**:
- Added a reviewable `AdventureExtractionDraft` pipeline for AI-first extraction of room graphs and trigger trees from source text.
- Extraction output is explicitly editable JSON-shaped data instead of opaque prompt text.
- Runtime intent handling now distinguishes location observation from actual movement.

**Residual gap**:
- Roll outcomes do not yet propagate through a rich trigger-tree engine; this remains the main follow-up after `v1.4`.

### Phase 18: 疯狂之馆 Room-Graph Migration
**Delivered**:
- `mad_mansion` now includes explicit location graph data for the central hall and each branch hall.
- Natural message play supports observing portals without forced entry, entering rooms intentionally, and returning to the central hall through location edges.
- Room navigation now feels more place-driven than sequence-driven.
